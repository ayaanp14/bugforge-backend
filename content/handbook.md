...
- A **language taught in depth**, not a problem list: a track per language, made of **modules** (chapters), each of several **lessons** and a **checkpoint**. Two tracks are open: **Java** (complete — 20 modules, 138 lessons, 292 programs, from the JVM and types through OOP, generics, collections, streams, memory, concurrency, I/O and modern Java to interview idioms) and **JavaScript** (in progress — the first 4 of a planned 16 modules: the runtime, values and types, functions and scope, objects and arrays; more modules are added to the same track, nothing done is lost). Python and C++ are listed as "in preparation" and cannot be started. The modules and lesson titles of a track are read from the site, not memory — point at `/study-plans/java` or `/study-plans/javascript`.
